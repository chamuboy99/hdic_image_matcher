...
import numpy as np
import torch
from PyQt5.QtWidgets import QApplication, QStackedLayout, QSizePolicy, QLabel, QPushButton, QVBoxLayout,QHBoxLayout, QWidget, QFileDialog, QMessageBox, QProgressDialog
...
class CameraApp(QWidget):
    def __init__(self):
        ...
        layout = QVBoxLayout()
        # Stacked layout for switching between camera and GIF
        self.stack = QStackedLayout()
        self.stack.addWidget(self.image_label)  # index 0
        self.stack.addWidget(self.paused_gif)   # index 1
    ...
    def update_frame(self):
        ...
        if self.processing or self.showing_match:
            # Show blank or loading or match screen while processing or showing match
            blank = np.zeros((480, 640, 3), dtype=np.uint8)
            if self.showing_match:
                # Draw match text on blank image
                cv2.putText(blank, self.result_label.text(), (20, 240),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            qt_image = QImage(blank.data, 640, 480, 640*3, QImage.Format_RGB888)
    ...
    def process_batch(self):
        ...
        if label is not None and score > 0.7:
            self.result_label.setStyleSheet("color: lightgreen; font-size: 16px;")
            self.result_label.setText(f"Match: {label} (Score: {score:.2f})")
        else:
            self.result_label.setStyleSheet("color: tomato; font-size: 16px;")
    ...
    def resume_feed(self):
        self.showing_match = False
        self.result_label.setStyleSheet("color: #aaaaaa; font-size: 16px;")
